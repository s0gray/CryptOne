...
public:
	...
	virtual ErrCode		hashData(const unsigned char *data, size_t dataLen, unsigned char *hashData) = 0;
	virtual ErrCode		hashPassword(const char *password, size_t passwordLen, char *hashedPassword)=0;
	...
	virtual size_t		getHashedPasswordLength() = 0;
	...
	uint16	generateUint16(); 
	virtual byte generateRndByte();	    
	...
	virtual ErrCode calcOneTimeAuthMac(const unsigned char* data, size_t dataLen, unsigned char* macData, const unsigned char* macKey)=0;
	virtual ErrCode calcOneTimeAuthMac2(const Blob& macKey, const Blob& dataToSign, Blob& mac)=0;
